...
class StockOverview
{
    ...
    public function readAll($allowedColumns)
    {
        ...
        try {
            $sql = "select ";
            // $sql = "select ms.*, ";
            $sql .= "MAX(p.products_low_stock_threshold), ";
            $sql .= "MAX(p.products_sku), ";
            $sql .= "MAX(p.products_unit), ";
            $sql .= "MAX(p.products_status), ";
            $sql .= "MAX(so.order_qty), ";
            $sql .= "MAX(p.products_aid), ";
            $sql .= "MAX(DATE_FORMAT(ms.stock_movement_date, '%b %d, %Y')) as stock_movement_date, ";
